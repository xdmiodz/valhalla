...
namespace valhalla {
namespace loki {
void loki_worker_t::parse_locations(
...
void loki_worker_t::parse_costing(valhalla_request_t &request) {
  ...
  if (!request.options.do_not_track()) {
    valhalla::midgard::logging::Log("costing_type::" + costing_str,
                                    " [ANALYTICS] ");
  }
  ...
  auto *method_options_ptr =
      rapidjson::Pointer{"/costing_options/" + costing_str}.Get(
          request.document);
  auto &allocator = request.document.GetAllocator();
  if (!method_options_ptr) {
    auto *costing_options =
        rapidjson::Pointer{"/costing_options"}.Get(request.document);
    if (!costing_options) {
  ...
  if (request.options.avoid_locations_size()) {
    try {
      auto avoid_locations =
          PathLocation::fromPBF(request.options.avoid_locations());
      auto results =
          loki::Search(avoid_locations, *reader, edge_filter, node_filter);
      ...
      for (const auto &result : results) {
        for (const auto &edge : result.second.edges) {
          ...
          GraphId shortcut;
          if (inserted.second &&
              (shortcut = reader->GetShortcut(edge.id)).Is_Valid()) {
...
loki_worker_t::loki_worker_t(
    ...
    : config(config), reader(graph_reader),
      ...
      sample(
          config.get<std::string>("additional_data.elevation", "test/data/")),
  ...
  if (action_str.empty()) {
    throw std::runtime_error(
        "The config actions for Loki are incorrectly loaded");
  }
  ...
  for (const auto &kv : config.get_child("service_limits")) {
    ...
    if (kv.first != "skadi" && kv.first != "trace" && kv.first != "isochrone") {
      max_matrix_distance.emplace(
          kv.first, config.get<float>("service_limits." + kv.first +
                                      ".max_matrix_distance"));
  ...
  min_transit_walking_dis = config.get<size_t>(
      "service_limits.pedestrian.min_transit_walking_distance");
  max_transit_walking_dis = config.get<size_t>(
      "service_limits.pedestrian.max_transit_walking_distance");

  max_avoid_locations =
      config.get<size_t>("service_limits.max_avoid_locations");
  max_reachability =
      config.get<unsigned int>("service_limits.max_reachability");
  default_reachability =
      config.get<unsigned int>("loki.service_defaults.minimum_reachability");
  ...
  max_search_radius =
      config.get<float>("service_limits.trace.max_search_radius");
  max_best_paths =
      config.get<unsigned int>("service_limits.trace.max_best_paths");
  max_best_paths_shape =
      config.get<size_t>("service_limits.trace.max_best_paths_shape");
...
worker_t::result_t
loki_worker_t::work(const std::list<zmq::message_t> &job, void *request_info,
  ...
  try {
    // request parsing
    auto http_request = http_request_t::from_string(
    ...
    if (!request.options.do_not_track() &&
        elapsed_time.count() / work_units > long_request) {
      LOG_WARN("loki::request elapsed time (ms)::" +
               std::to_string(elapsed_time.count()));
      LOG_WARN("loki::request exceeded threshold::" +
...
void run_service(const boost::property_tree::ptree &config) {
  ...
  auto downstream_endpoint =
      config.get<std::string>("thor.service.proxy") + "_in";
  ...
  prime_server::worker_t worker(
      context, upstream_endpoint, downstream_endpoint, loopback_endpoint,
      interrupt_endpoint,
