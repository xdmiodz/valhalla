...
const std::unordered_map<unsigned, unsigned> ERROR_TO_STATUS{
    ...
    {120, 400}, {121, 400}, {122, 400}, {123, 400}, {124, 400}, {125, 400},
    {126, 400},
    ...
    {150, 400}, {151, 400}, {152, 400}, {153, 400}, {154, 400}, {155, 400},
    {156, 400}, {157, 400}, {158, 400},
...
rapidjson::Document from_string(const std::string &json,
                                const valhalla_exception_t &e) {
...
void parse_locations(
  ...
  auto request_locations =
      rapidjson::get_optional<rapidjson::Value::ConstArray>(
          doc, std::string("/" + node).c_str());
  if (request_locations) {
    for (const auto &r_loc : *request_locations) {
      try {
        ...
        auto stop_type_json =
            rapidjson::get_optional<std::string>(r_loc, "/type");
        ...
        auto date_time =
            rapidjson::get_optional<std::string>(r_loc, "/date_time");
        if (date_time) {
        ...
        auto heading_tolerance =
            rapidjson::get_optional<int>(r_loc, "/heading_tolerance");
        if (heading_tolerance) {
          location->set_heading_tolerance(*heading_tolerance);
        }
        auto node_snap_tolerance =
            rapidjson::get_optional<float>(r_loc, "/node_snap_tolerance");
        ...
        auto minimum_reachability = rapidjson::get_optional<unsigned int>(
            r_loc, "/minimum_reachability");
        ...
        auto accuracy =
            rapidjson::get_optional<unsigned int>(r_loc, "/accuracy");
        if (accuracy) {
        ...
        auto rank_candidates =
            rapidjson::get_optional<bool>(r_loc, "/rank_candidates");
        if (rank_candidates) {
    ...
    if (track) {
      midgard::logging::Log(
          node + "_count::" + std::to_string(request_locations->Size()),
          " [ANALYTICS] ");
    }
  }
}
...
void parse_contours(
  ...
  auto json_contours =
      rapidjson::get_optional<rapidjson::Value::ConstArray>(doc, "/contours");
  if (json_contours) {
    float prev = 0.f;
    const float NO_TIME = -1.f;
    for (const auto &json_contour : *json_contours) {
      // Grab contour time and validate that it is increasing
      const float c = rapidjson::get_optional<float>(json_contour, "/time")
                          .get_value_or(NO_TIME);
      if (c < prev || c == NO_TIME) {
...
void from_json(rapidjson::Document &doc, odin::DirectionsOptions &options) {
  ...
  if (language &&
      odin::get_locales().find(*language) != odin::get_locales().end()) {
  ...
  auto encoded_polyline =
      rapidjson::get_optional<std::string>(doc, "/encoded_polyline");
  if (encoded_polyline) {
    options.set_encoded_polyline(*encoded_polyline);
    auto decoded =
        midgard::decode<std::vector<midgard::PointLL>>(*encoded_polyline);
  ...
  // TODO: remove this?
  options.set_do_not_track(
      rapidjson::get_optional<bool>(doc, "/healthcheck").get_value_or(false));
  ...
  for (const auto costing :
       {odin::auto_, odin::auto_shorter, odin::bicycle, odin::bus, odin::hov,
    ...
    switch (costing) {
    case odin::auto_: {
      sif::ParseAutoCostOptions(doc, costing_options_key,
                                options.add_costing_options());
      break;
    }
    case odin::auto_shorter: {
      sif::ParseAutoShorterCostOptions(doc, costing_options_key,
                                       options.add_costing_options());
      break;
    }
    case odin::bicycle: {
      sif::ParseBicycleCostOptions(doc, costing_options_key,
                                   options.add_costing_options());
      break;
    }
    case odin::bus: {
      sif::ParseBusCostOptions(doc, costing_options_key,
                               options.add_costing_options());
      break;
    }
    case odin::hov: {
      sif::ParseHOVCostOptions(doc, costing_options_key,
                               options.add_costing_options());
      break;
    }
    case odin::motor_scooter: {
      sif::ParseMotorScooterCostOptions(doc, costing_options_key,
                                        options.add_costing_options());
      break;
    }
    ...
    case odin::pedestrian: {
      sif::ParsePedestrianCostOptions(doc, costing_options_key,
                                      options.add_costing_options());
      break;
    }
    case odin::transit: {
      sif::ParseTransitCostOptions(doc, costing_options_key,
                                   options.add_costing_options());
      break;
    }
    case odin::truck: {
      sif::ParseTruckCostOptions(doc, costing_options_key,
                                 options.add_costing_options());
      break;
    }
    case odin::motorcycle: {
      sif::ParseMotorcycleCostOptions(doc, costing_options_key,
                                      options.add_costing_options());
      break;
    }
    case odin::auto_data_fix: {
      sif::ParseAutoDataFixCostOptions(doc, costing_options_key,
                                       options.add_costing_options());
      break;
    }
    }
  ...
  if (date_time_type &&
      odin::DirectionsOptions::DateTimeType_IsValid(*date_time_type)) {
    auto const v = static_cast<int>(*date_time_type);
    options.set_date_time_type(
        static_cast<odin::DirectionsOptions::DateTimeType>(v));
  } // not specified but you want transit, then we default to current
  else if (options.has_costing() && (options.costing() == odin::multimodal ||
                                     options.costing() == odin::transit)) {
  ...
  if (options.has_date_time_type()) {
    ...
    case odin::DirectionsOptions::arrive_by:
      // not yet for transit
      if (options.costing() == odin::multimodal ||
          options.costing() == odin::transit) {
  ...
  // get some parameters
  auto resample_distance =
      rapidjson::get_optional<double>(doc, "/resample_distance");
  ...
  auto shape_match_str =
      rapidjson::get_optional<std::string>(doc, "/shape_match");
  ...
  auto gps_accuracy =
      rapidjson::get_optional<float>(doc, "/trace_options/gps_accuracy");
  if (gps_accuracy) {
  ...
  auto search_radius =
      rapidjson::get_optional<float>(doc, "/trace_options/search_radius");
  if (search_radius) {
  ...
  auto breakage_distance =
      rapidjson::get_optional<float>(doc, "/trace_options/breakage_distance");
  if (breakage_distance) {
    options.set_breakage_distance(*breakage_distance);
  }

  // if specified, get the filter_action value in there
  auto filter_action_str =
      rapidjson::get_optional<std::string>(doc, "/filters/action");
  odin::FilterAction filter_action;
  if (filter_action_str &&
      valhalla::odin::FilterAction_Parse(*filter_action_str, &filter_action)) {
  ...
  doc.AddMember({"language", allocator}, {options.language(), allocator},
                allocator);
  doc.AddMember(
      {"format", allocator},
      {valhalla::odin::DirectionsOptions_Format_Name(options.format()),
       allocator},
      allocator);
}
...
namespace odin {
bool DirectionsOptions_Action_Parse(const std::string &action,
                                    odin::DirectionsOptions::Action *a) {
  static const std::unordered_map<std::string, odin::DirectionsOptions::Action>
      actions{
...
const std::string &
DirectionsOptions_Action_Name(const odin::DirectionsOptions::Action action) {
...
bool DirectionsOptions_Format_Parse(const std::string &format,
                                    odin::DirectionsOptions::Format *f) {
  static const std::unordered_map<std::string, odin::DirectionsOptions::Format>
      formats{
...
const std::string &
DirectionsOptions_Format_Name(const odin::DirectionsOptions::Format match) {
  static const std::string empty;
...
const std::string &
DirectionsOptions_Units_Name(const odin::DirectionsOptions::Units unit) {
  static const std::string empty;
...
valhalla_exception_t::valhalla_exception_t(
    unsigned code, const boost::optional<std::string> &extra)
  ...
  http_code =
      (http_code_iter == ERROR_TO_STATUS.cend() ? 0 : http_code_iter->second);
  auto http_message_iter = HTTP_STATUS_CODES.find(http_code);
  http_message = (http_message_iter == HTTP_STATUS_CODES.cend()
                      ? ""
                      : http_message_iter->second);
}

valhalla_request_t::valhalla_request_t() { document.SetObject(); }
void valhalla_request_t::parse(const std::string &request,
...
void valhalla_request_t::parse(const http_request_t &request) {
  ...
  for (const auto &kv : request.query) {
    ...
    if (kv.first == "json" || kv.first.empty() || kv.second.empty() ||
        kv.second.front().empty()) {
      continue;
    }

    // turn single value entries into single key value
    if (kv.second.size() == 1) {
      document.AddMember({kv.first, allocator}, {kv.second.front(), allocator},
                         allocator);
      continue;
    }
...
const headers_t::value_type GPX_MIME{"Content-type",
                                     "application/gpx+xml;charset=utf-8"};
const headers_t::value_type ATTACHMENT{"Content-Disposition",
...
worker_t::result_t to_response_xml(const std::string &xml,
                                   ...
                                   const valhalla_request_t &request) {
  worker_t::result_t result{false};
  http_response_t response(200, "OK", xml,
                           headers_t{CORS, GPX_MIME, ATTACHMENT});
  response.from_info(request_info);
  result.messages.emplace_back(response.to_string());
  return result;
}
...
void service_worker_t::set_interrupt(
